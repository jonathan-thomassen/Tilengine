...
static void apply_palette(TLN_Tilemap palette_layer, TLN_Tilemap target_layer) {
  const int numtiles =
      TLN_GetTilemapCols(palette_layer) * TLN_GetTilemapRows(palette_layer);
...
int main(int argc, char *argv[]) {
  ...
  TLN_Palette global_palette1 = TLN_ClonePalette(
      TLN_GetTilesetPalette(TLN_GetTilemapTileset(layer_foreground)));
  ...
  const int numtiles = TLN_GetTilemapCols(layer_foreground) *
                       TLN_GetTilemapRows(layer_foreground);
