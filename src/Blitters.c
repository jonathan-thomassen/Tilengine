 ...
 * blending */
static void blitFastBlendScaling_8_32(const uint8_t *srcpixel,
...
static void blitKeyBlendScaling_8_32(const uint8_t *srcpixel,
                                     TLN_Palette palette, void *dstptr,
                                     int width, int dx, int offset,
                                     const uint8_t *blend) {
  uint8_t const *src;
...
static const ScanBlitPtr blitters[] = {
    blitFast_8_32,        blitFastBlend_8_32,
    blitFastScaling_8_32, blitFastBlendScaling_8_32,
    blitKey_8_32,         blitKeyBlend_8_32,
    blitKeyScaling_8_32,  blitKeyBlendScaling_8_32};

/* returns suitable blitter for specified conditions */
ScanBlitPtr SelectBlitter(bool key, bool scaling, bool blend) {
  int index = ((int)key << BLIT_KEY) + ((int)scaling << BLIT_SCALING) +
              ((int)blend << BLIT_BLEND);
...
static void PaintMosaicBlockRegular(Color const *srcpixel, Color *dstpixel,
                                    int block) {
  while (block != 0) {
    dstpixel->value = srcpixel->value;
    dstpixel += 1;
