...
int initializeSimpleXmlParser(SimpleXmlParserState parser, const char *sData,
                              long nDataSize);
const char *
getSimpleXmlParseErrorDescription(struct simplexml_parser_state const *parser);
...
int simpleXmlInitializeParser(SimpleXmlParser parser, const char *sData,
                              long nDataSize) {
...
SimpleXmlParserState createSimpleXmlParser(const char *sData, long nDataSize) {
  if (sData != NULL && nDataSize > 0) {
    SimpleXmlParserState parser =
        (SimpleXmlParserState)malloc(sizeof(TSimpleXmlParserState));
    if (parser == NULL) {
      return NULL;
    }
...
int parseOneTag(SimpleXmlParserState parser,
  ...
  szTagName =
      mystrdup(getInternalSimpleXmlValueBufferContents(parser->vbNextToken));
  if (szTagName == NULL) {
    parser->nError = OUT_OF_MEMORY;
    return FAIL;
  }
  ...
  handler = (SimpleXmlTagHandler)parentHandler(
      (SimpleXmlParser)parser, ADD_SUBTAG, szTagName, NULL, NULL);
  ...
  while (parser->nNextToken != TAG_END &&
    ...
    if (parser->nNextToken == ATTRIBUTE) {
      ...
      handler((SimpleXmlParser)parser, ADD_ATTRIBUTE, szTagName,
              parser->szAttribute,
  ...
  if (parser->nNextToken == TAG_BEGIN_CLOSING) {
    ...
    if (strcmp(szTagName, getInternalSimpleXmlValueBufferContents(
  ...
  if (peekInputChar(parser) == '/') {
    ...
    if (parser->szAttribute == NULL ||
        parser->nAttributeBufferSize <
            getSimpleXmlValueBufferContentLength(parser->vbNextToken)) {
      if (parser->szAttribute != NULL) {
        free(parser->szAttribute);
      }
      parser->nAttributeBufferSize =
...
int readNextContentToken(SimpleXmlParserState parser) {
  ...
  if (peekInputChar(parser) == '<') {
    ...
    if (peekInputChar(parser) == '/') {
      ...
      parser->nNextToken = TAG_END;
      skipInputChar(parser);
      while (peekInputChar(parser) > SPACE && peekInputChar(parser) != '>') {
        /* read one character into next token value buffer */
        if (readChar(parser) == FAIL) {
          return FAIL;
        }
 ...
 */
SimpleXmlValueBuffer createSimpleXmlValueBuffer(long nInitialSize) {
  SimpleXmlValueBuffer vb =
      (SimpleXmlValueBuffer)malloc(sizeof(TSimpleXmlValueBuffer));
  if (vb == NULL) {
    return NULL;
  }
