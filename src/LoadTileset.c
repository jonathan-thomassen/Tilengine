...
static void *handler(SimpleXmlParser /*parser*/, SimpleXmlEvent evt,
  ...
  case FINISH_ATTRIBUTES:
    if (!strcasecmp(szName, "tileset") && loader.tilecount != 0) {
      loader.attributes = (TLN_TileAttributes *)calloc(
          loader.tilecount, sizeof(TLN_TileAttributes));
      loader.images =
  ...
  case FINISH_TAG:
    if (!strcasecmp(szName, "frame"))
      ...
      sequence = TLN_CreateSequence(name, loader.tile.id + 1,
...
TLN_Tileset TLN_LoadTileset(const char *filename) {
  ...
  if (loader.source[0] != 0) {
    ...
    tileset = TLN_CreateTileset(tilecount, loader.tilewidth, loader.tileheight,
