...
void ccm_prepare_first_ctr_blk(uint8_t counter[], const uint8_t nonce[],
...
void ccm_format_assoc_data(uint8_t buf[], int *end_of_buf,
                           const uint8_t assoc[], int assoc_len);
void ccm_format_payload_data(uint8_t buf[], int *end_of_buf,
 ...
 * AES - CBC
 *******************/
int aes_encrypt_cbc(const uint8_t in[], uint32_t in_len, uint8_t out[],
                    const uint32_t key[], int keysize, const uint8_t iv[]) {
...
int aes_encrypt_cbc_mac(const uint8_t in[], uint32_t in_len, uint8_t out[],
                        const uint32_t key[], int keysize, const uint8_t iv[]) {
  uint8_t buf_in[AES_BLOCK_SIZE];
  uint8_t buf_out[AES_BLOCK_SIZE];
  uint8_t iv_buf[AES_BLOCK_SIZE];
...
int aes_decrypt_cbc(const uint8_t in[], uint32_t in_len, uint8_t out[],
                    const uint32_t key[], int keysize, const uint8_t iv[]) {
  uint8_t buf_in[AES_BLOCK_SIZE];
  uint8_t buf_out[AES_BLOCK_SIZE];
  uint8_t iv_buf[AES_BLOCK_SIZE];
...
void increment_iv(uint8_t iv[], int counter_size) {
  ...
  // increment.
  for (int idx = AES_BLOCK_SIZE - 1; idx >= AES_BLOCK_SIZE - counter_size;
       idx--) {
    iv[idx]++;
...
void aes_encrypt_ctr(const uint8_t in[], uint32_t in_len, uint8_t out[],
                     const uint32_t key[], int keysize, const uint8_t iv[]) {
  uint32_t idx = 0;
...
void aes_decrypt_ctr(const uint8_t in[], uint32_t in_len, uint8_t out[],
                     const uint32_t key[], int keysize, const uint8_t iv[]) {
...
int aes_encrypt_ccm(const uint8_t payload[], uint32_t payload_len,
                    const uint8_t assoc[], unsigned short assoc_len,
                    const uint8_t nonce[], unsigned short nonce_len,
                    uint8_t out[], uint32_t *out_len, uint32_t mac_len,
...
int aes_decrypt_ccm(const uint8_t ciphertext[], uint32_t ciphertext_len,
                    const uint8_t assoc[], unsigned short assoc_len,
                    const uint8_t nonce[], unsigned short nonce_len,
                    uint8_t plaintext[], uint32_t *plaintext_len,
                    uint32_t mac_len, int *mac_auth, const uint8_t key_str[],
                    int keysize) {
  uint8_t temp_iv[AES_BLOCK_SIZE];
  uint8_t counter[AES_BLOCK_SIZE];
  uint8_t mac[16];
...
void ccm_prepare_first_format_blk(uint8_t buf[], int assoc_len, int payload_len,
  ...
  buf[0] = (uint8_t)(((((mac_len - 2) / 2) & 0x07) << 3) |
...
void aes_encrypt(const uint8_t in[], uint8_t out[], const uint32_t key[],
                 int keysize) {
  uint8_t state[4][4];
...
void aes_decrypt(const uint8_t in[], uint8_t out[], const uint32_t key[],
                 int keysize) {
  uint8_t state[4][4];
