...
// identity matrix
static math2d_t identity3[] = {1, 0, 0, 0, 1, 0, 0, 0, 1};

// initialize matrix with specified values
...
void Matrix3SetScale(Matrix3 *matrix, math2d_t sx, math2d_t sy) {
  math2d_t data[] = {sx, 0, 0, 0, sy, 0, 0, 0, 1};
  Matrix3Set(matrix, data);
}
...
void Matrix3Multiply(Matrix3 *matrix, const Matrix3 *mul) {
  ...
  tmp.m11 =
      mul->m11 * matrix->m11 + mul->m12 * matrix->m21 + mul->m13 * matrix->m31;
  tmp.m12 =
      mul->m11 * matrix->m12 + mul->m12 * matrix->m22 + mul->m13 * matrix->m32;
  tmp.m13 =
      mul->m11 * matrix->m13 + mul->m12 * matrix->m23 + mul->m13 * matrix->m33;

  tmp.m21 =
      mul->m21 * matrix->m11 + mul->m22 * matrix->m21 + mul->m23 * matrix->m31;
  tmp.m22 =
      mul->m21 * matrix->m12 + mul->m22 * matrix->m22 + mul->m23 * matrix->m32;
  tmp.m23 =
      mul->m21 * matrix->m13 + mul->m22 * matrix->m23 + mul->m23 * matrix->m33;

  tmp.m31 =
      mul->m31 * matrix->m11 + mul->m32 * matrix->m21 + mul->m33 * matrix->m31;
  tmp.m32 =
      mul->m31 * matrix->m12 + mul->m32 * matrix->m22 + mul->m33 * matrix->m32;
  tmp.m33 =
      mul->m31 * matrix->m13 + mul->m32 * matrix->m23 + mul->m33 * matrix->m33;
