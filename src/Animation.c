...
static inline void blendColors(uint8_t const *srcptr0, uint8_t const *srcptr1, uint8_t *dstptr,
...
static void UpdatePaletteStrip(Animation *animation, struct Strip *strip, int time);
...
void UpdateAnimation(Animation *animation, int time) {
    ...
    case TYPE_TILESET:
        animation->tileset->tiles[sequence->target] = (uint16_t)frames[animation->pos].index;
        break;
...
static void ColorCycleBlend(TLN_Palette srcpalette, TLN_Palette dstpalette,
    ...
    for (int c = 0; c < count; c++) {
        ...
        uint8_t const *srcptr0 = (uint8_t *)GetPaletteData(srcpalette, strip->first + idx0);
        uint8_t const *srcptr1 = (uint8_t *)GetPaletteData(srcpalette, strip->first + idx1);
