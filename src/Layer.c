...
/*!
 * \brief Configures a background layer with a object list and an image-based
 * tileset
 *
 * \param nlayer Layer index [0, num_layers - 1]
 * \param objects Reference to the TLN_ObjectList to attach
 * \param tileset optional reference to the image-based tileset object. If NULL,
 ...
 *
 * \remarks
 * Call this function inside a raster callback to change the palette in the
 * middle of the frame to get raster effect colors, like and "underwater"
 * palette below the water line in a partially submerged background, or a
 * gradient palette in an area at the top of the screen to simulate a "depth fog
...
/*!
 * \brief Returns the active palette of a layer if set with \ref
 * TLN_SetLayerPalette(), or the palette of the first tileset, or palette of
 * bitmap
 * \param nlayer Layer index [0, num_layers - 1]
 ...
 * function to move the viewport insde the tileset. Change this value
 * progressively for each frame to get a scrolling effect
 *
 * \remarks
 ...
 * layer that move at different speeds to simulate depth. The extreme case of
 * this effect, where the position is changed in each scanline, is called "line
 * scroll" and was the technique used by games such as Street Fighter II to
 ...
 *
 * \param info
 * Pointer to an application-allocated TLN_TileInfo struct that will get the
 * data
 *
 * \returns
 ...
 *
 * \remarks
 * Use this function to implement collision detection between sprites and the
 ...
 *
 * \remarks
 * This feature is tipically used to simulate vertical strips moving at
 * different speeds, or combined with a line scroll effect, to fake rotations
 * where the angle is small. The Sega Genesis games Puggsy and Chuck Rock II
 ...
 * \param nlayer Layer index [0, num_layers - 1]
 * \remarks The layer must have been previously configured. A layer without a
 ...
 *
 * Enable the transformation matrix to give the layer the capabilities of the
 * famous Super Nintendo / Famicom Mode 7. Beware that the rendering of a
 * transformed layer uses more CPU than a regular layer. Unlike the original
 * Mode 7, that could only transform the single layer available, Tilengine can
 * transform all the layers at the same time. The only limitation is the
 * available CPU power.
 *
 * \remarks
 * Call this function inside a raster callback to set the transformation matrix
 * in the middle of the frame. Setting it for each scanline is the trick used by
 * many Super Nintendo games to fake a 3D perspective projection.
 *
 * \see
 ...
 *
 * \remarks
 * This function is a simple wrapper to TLN_SetLayerAffineTransform() without
 * using the TLN_Affine struct
 *
 * \see
 ...
 * no scaling. Use values below 1.0 to downscale (shrink) and above 1.0 to
 * upscale (enlarge). Call TLN_ResetLayerMode() to disable scaling
 *
 ...
 * When color is enabled on window, the area outside the clipped region gets
 * filled with this color. If one of blending modes is selected, color math is
