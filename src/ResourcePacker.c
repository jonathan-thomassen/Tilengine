...
static uint8_t iv[AES_BLOCK_SIZE] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
...
static void *load_asset(ResPack rp, ResEntry const *entry) {
  ...
  if (rp->encrypted == true) {
    ...
    if (cyphertext != NULL && plaintext != NULL) {
      ...
      aes_decrypt_cbc((uint8_t *)cyphertext, entry->pack_size,
                      (uint8_t *)plaintext, rp->key, KEY_SIZE, iv);
...
int ResPack_Build(const char *filelist, const char *passphrase) {
  ...
  if (pf_output == NULL) {
    fclose(pf_list);
    printf("ResPack_Build error: cannot create output file %s\n", filename);
    return 0;
  }
  ...
  for (c = 0; c < res_header.num_regs; c++) {
    ...
    if (passphrase != NULL) {
      ...
      uint32_t pack_size =
          (entry->data_size + AES_BLOCK_SIZE - 1) & ~(AES_BLOCK_SIZE - 1);
      ...
      aes_encrypt_cbc(plaintext, entry->pack_size, (uint8_t *)cyphertext, key,
                      KEY_SIZE, iv);
      content = cyphertext;
      free(plaintext);
    }
