 ...
 *
 * \param sp
 * Optional reference to the optional sequence pack with associated tileset
 ...
 *
 * \remarks
 * Care must be taken in providing pixel data and pitch as it can crash the
 * aplication
 *
 * \see
 * TLN_CreateTileset()
 ...
 *
 * \remarks
 * The palette of a tileset is created at load time and cannot be modified. When
 * TLN_SetLayer function is used to attach a tileset to a layer, the palette
 * associated with the specified tileset is automatically assigned to that
 * layer, but it can be later replaced with TLN_SetLayerPalette
 *
 * \see
...
/*!
 * \brief
 * Returns a reference to the optional sequence pack associated to the specified
 * tileset
 *
 * \param tileset
 * Reference to the tileset to get the palette
