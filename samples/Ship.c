...
void ShipTasks(Actor *actor) {
  Ship const *ship = (Ship *)actor->usrdata;
...
void ShotTasks(Actor *actor) {
  ...
  for (c = ACTOR_ENEMY1; c < last; c++) {
    ...
    if (target->state != 0 && target->type == TYPE_ENEMY &&
        CheckActorCollision(actor, target)) {
      HitEnemy(target, power);
      {
        CreateExplosion(actor->index, actor->x, actor->y,
  ...
  for (c = ACTOR_BOSS; c < last; c++) {
    ...
    if (target->state != 0 && CheckActorCollision(actor, target)) {
      CreateExplosion(actor->index, actor->x, actor->y, sequences[SEQ_EXPLO2]);
      actor->vx = target->vx;
      actor->vy = target->vy;
      return;
    }
