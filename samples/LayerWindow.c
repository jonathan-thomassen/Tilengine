...
/* layers */
enum { LAYER_FOREGROUND, LAYER_BACKGROUND, NUM_LAYERS };
...
void update_window(void) {
  TLN_SetLayerWindow(LAYER_FOREGROUND, window_x, window_y,
                     window_x + window_width, window_y + window_height,
                     state.invert);
  if (state.color)
    TLN_SetLayerWindowColor(LAYER_FOREGROUND, 0, 128, 224,
                            state.blend ? BLEND_MIX : BLEND_NONE);
  else
